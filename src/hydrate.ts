import type { Context, MessageEntity, NextFunction, ParseMode } from './deps.deno.ts';
import type { Stringable } from './format.ts';
...
const buildReplyWithParseMode = <C extends ParseModeContext>(parseMode: ParseMode, ctx: C) => {
...
const middleware = async <C extends ParseModeContext>(ctx: C, next: NextFunction) => {
  ctx.replyFmt = (stringLike, ...args) => {
    ...
    const entities = stringLike instanceof FormattedString ? { entities: stringLike.entities } : undefined;
    return ctx.reply(stringLike.toString(), { ...payload, ...entities }, ...rest as any);
  };
